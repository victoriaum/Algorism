...
```
SELECT * FROM(
SELECT P.PRODUCT_ID, PRODUCT_NAME, PRICE*SUM(AMOUNT) AS TOTAL
...
+ 문제 잘 읽기, 가장 많은 리뷰를 남긴 사람에 대해서 출력되어야 한다.
+ 이 코드는 단점이 있는데 가장 많은 리뷰를 남긴 사람이 동점으로 여러명이라면 LIMIT 처리로 가져오는 건 1명에 해당해서 조건을 충족한다고 볼 수 없다.
+ 답은 맞지만, 답은 아닌 코드..
```
...
+ 총계를 구할때는 항상 겹치는 이름이 있는지 확인해야 한다.
```
SELECT A.FLAVOR
...
### 조건별로 분류하여 주문상태 출력하기
```
SELECT ORDER_ID, PRODUCT_ID, IFNULL(OUT_DATE,'') AS OUT_DATE, IF(OUT_DATE IS NULL, '출고미정', IF(OUT_DATE>STR_TO_DATE('20220501', '%Y%m%d'),'출고대기','출고완료')) AS '출고여부'
FROM FOOD_ORDER
ORDER BY ORDER_ID
```
...
WHERE APNT_YMD LIKE '2022-04-13%' AND APNT_CNCL_YN!='Y'
ORDER BY A.APNT_YMD;
```
...
FROM(
SELECT I.ANIMAL_ID, I.NAME, O.DATETIME-I.DATETIME AS GAP
FROM ANIMAL_INS I JOIN ANIMAL_OUTS O 
ON I.ANIMAL_ID = O.ANIMAL_ID
) AS T
ORDER BY GAP DESC LIMIT 0,2
```